...
def read_message(message_id):
    ...
    user_id = session['user_id']
    # VULN V-06/V-07 pattern: sem verificação de que recipient_id == user_id
    msg = unsafe_query(
...
def compose():
    ...
    if request.method == 'POST':
        ...
        # VULN V-02: body armazenado sem sanitização — stored XSS via mensagens
        recipient = unsafe_query(f"SELECT id FROM users WHERE username='{recipient_username}'")
        if recipient:
            execute_db(
